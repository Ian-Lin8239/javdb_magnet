...
### v1.4.0 (2026-01-17)
- ✨ **改進防重複機制**
  - 添加番號格式驗證功能，自動過濾異常格式的番號（如 `Mm2QnR`、`bKXY90` 等）
  - 支援多種正常番號格式：`SSIS-886-C`、`JUR-496`、`300MIUM-1273` 等
  - 載入時自動清理現有異常番號，確保數據庫質量
  
- 📈 **提升記錄容量**
  - 記錄限制從 300 筆大幅提升至 10000 筆
  - 超過限制時自動清理最舊的記錄，保留最新的 10000 筆
  
- 🛡️ **增強數據驗證**
  - 記錄前驗證番號格式，只保存有效格式的番號
  - 檢查重複時只比對有效格式的番號，提高準確性
  - 統計資訊新增有效/無效番號數量顯示
